...
from TEMPENCDEC import Im2LatexModel
...
def MGD(net, net2, train_dataloader, learning_rate, momentum, n_epochs):
    criterion = nn.CrossEntropyLoss() # Ändra denna?
    
    optimizer = optim.SGD(net.parameters(), lr=learning_rate, momentum=momentum)  # Should use Adam instead
...
def main():
    ...
    net2 = Im2LatexModel(out_size=vocab_size, emb_size = embedding_size, dec_rnn_h = hidden_size)

    ED_Trained = MGD(ED, net2, train_loader, learning_rate=0.001, momentum=0.9, n_epochs=10)

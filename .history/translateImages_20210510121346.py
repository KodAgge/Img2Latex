...
out_folder_suffix = '_transformed/' # EK) I changed this to "_transformed/" so that your data wouldn't be overwritten when I did some tests.  
...
for entry in os.scandir(img_folder):
    if (entry.path.endswith(".inkml") or entry.path.endswith(".inmkl")) and entry.is_file():
        ...
        inkml2img.inkml2img(entry.path, r'./data/CROHME DATA/' + mode + final_folder_suffix + img_name)
